...
* 본 시스템은 실제 네트워크 서버 및 외부 DB 환경을 연동하지 않는 백엔드 중심 구동 체계이므로, 네트워크 연결이 지원되지 않는 오프라인 로컬 데스크탑 환경에서도 모든 핵심 기능이 독립적으로 구동될 수 있어야 한다.
* 실제 데이터베이스 서버가 아닌 로컬 파일 입출력 방식을 사용하므로, 메모리 내에 객체화된 데이터 목록을 효율적으로 탐색하고 필터링하는 로직을 구현하여 메모리 낭비를 최소화한다.
